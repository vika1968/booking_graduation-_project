...
import { hotelInputs, roomsType } from "../../../formSource";
 ...
 // const [files, setFiles] = useState<File[]>([]);
  ...
  const handleClick = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {   
      ...
      if(info.distance === undefined || isNaN(Number(info.distance)) ||  Number(info.distance) ===0) {
    ...
    hotelInputs.forEach((input) => {
      if ((input.id === info.name && (info.name === undefined || info.name.trim()==='')) || 
       (input.id === info.type && ( info.type === undefined || info.type.trim()==='')) || 
       (input.id === info.city && ( info.city === undefined || info.city.trim()==='')) || 
       (input.id === info.address && ( info.address === undefined || info.address.trim()==='')) || 
       (input.id === info.distance && ( info.distance === undefined || info.distance.trim()==='')) ||
